...
use Civi\Osdi\PersonBatchSyncerInterface;
use Civi\Osdi\SyncResult;

class N2FBatch implements PersonBatchSyncerInterface {

  private ?PersonSingleSyncerInterface $singleSyncer;

  public function getSingleSyncer(): ?PersonSingleSyncerInterface {
    return $this->singleSyncer;
  }

  public function setSingleSyncer(PersonSingleSyncerInterface $singleSyncer): void {
    $this->singleSyncer = $singleSyncer;
  }

  public function __construct(PersonSingleSyncerInterface $singleSyncer = NULL) {
    $this->singleSyncer = $singleSyncer;
  }
  ...
  public function batchSyncFromRemote(): ?int {
    ...
    if (is_null(\Civi::settings()->get('ntfActionNetwork.syncJobEndTime'))) {
  ...
  private function findAndSyncRemoteUpdatesAsNeeded($cutoff): \Civi\Osdi\ResultCollection {
    ...
    foreach ($searchResults as $remotePerson) {
      ...
      try {
        $syncResult = $this->singleSyncer->syncFromRemoteIfNeeded($remotePerson);
      }
      catch (\Throwable $e) {
  ...
  private function findAndSyncLocalUpdatesAsNeeded($cutoff): array {
    ...
    foreach ($civiEmails as $i => $emailRecord) {
      ...
      $syncResult = $this->singleSyncer->syncFromLocalIfNeeded($localPerson);
  ...
  private function getOrCreateActNetModTimeHorizon() {
    $cutoff = \Civi::settings()
      ->get('ntfActionNetwork.syncJobActNetModTimeCutoff');
  ...
  private function getOrCreateLocalModTimeHorizon() {
    $cutoff = \Civi::settings()
      ->get('ntfActionNetwork.syncJobCiviModTimeCutoff');
    $cutoffWasRetrievedFromPreviousSync = !empty($cutoff);
    Logger::logDebug('Horizon time was ' .
      ($cutoffWasRetrievedFromPreviousSync ? '' : 'not') .
      'retrieved from the previous sync'
